...
public class HandlerMethod {
	...
	/**
	 * Create an instance from a bean instance, method name, and parameter
	 * types.
	 *
	...
	public HandlerMethod(String beanName, BeanFactory beanFactory, Method method) {
		...
		Assert.notNull(method, "method is required");
		Assert.isTrue(beanFactory.containsBean(beanName), "Bean factory [" + beanFactory + "] does not contain bean ["
				+ beanName + "]");
		this.bean = beanName;
	...
	/**
	 * Returns the type of the handler for this handler method. Note that if the
	 * bean type is a CGLIB-generated class, the original, user-defined class is
	 * returned.
	 */
	public Class<?> getBeanType() {
		Class<?> clazz = (this.bean instanceof String) ? this.beanFactory.getType((String) this.bean) : this.bean
				.getClass();
	...
	/**
	 * Returns {@code true} if the method return type is void, {@code false}
	 * otherwise.
	 */
	...
	/**
	 * Returns a single annotation on the underlying method traversing its super
	 * methods if no annotation can be found on the given method itself.
	 *
	...
	/**
	 * If the provided instance contains a bean name rather than an object
	 * instance, the bean name is resolved before a {@link HandlerMethod} is
	...
	/**
	 * A MethodParameter for a HandlerMethod return type based on an actual
	 * return value.
	 */
