...
/**
 * Comparator capable of sorting exceptions based on their depth from the thrown
 * exception type.
 *
 ...
 * @since 3.0.3
 */
public class ExceptionDepthComparator implements
	...
	private int getDepth(@SuppressWarnings("rawtypes") Class declaredException,
			@SuppressWarnings("rawtypes") Class exceptionToMatch, int depth) {
		...
		return getDepth(declaredException, exceptionToMatch.getSuperclass(),
				depth + 1);
	}

	/**
	 * Obtain the closest match from the given exception types for the given
	 * target exception.
	 *
	...
	public static Class<? extends Throwable> findClosestMatch(
		...
		List<Class<? extends Throwable>> handledExceptions = new ArrayList<Class<? extends Throwable>>(
				exceptionTypes);
		Collections.sort(handledExceptions, new ExceptionDepthComparator(
				targetException));
