...
public class AtomikosTxTest {
	@Value("#{dbcp_JdbcTemplate}")
	private JdbcTemplate dbcpJdbcTemplate;
	@Value("#{spring_JdbcTemplate}")
	...
	public void AutoCommit確認() throws Exception {
		boolean isAutoCommit = isAutoCommit(springJdbcTemplate);
		...
		List<SampleVo> list = template.execute(new TransactionCallback<List<SampleVo>>() {
			public List<SampleVo> doInTransaction(TransactionStatus status) {
				...
				return jdbcTemplate.query(query, new Object[] { id },
						new BeanPropertyRowMapper<SampleVo>(SampleVo.class));
			}
		});
